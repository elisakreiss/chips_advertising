...
Choosing a **price threshold**: The first decision to make is, whether to consider the price per 100g or the total price. I decided for the total price, because I believe that this is in the end what people base their decision on. However, it should be noted, that this decision only changes the categorization of one item. <br> <br>
A tiny anecdote <br>
...
For one, the amount of translations varys a lot between potato chips bags. Some bags do not have any translations and others have whole advertisement paragraphs phrased in up to four and ingredients in up to eleven languages. The amount of translations on a bag therefore highly influence how much space is left for additional advertisement. <br>
...
- add sentence complexity/length measure (Flesch-Kincaid readability test: weighted value of the length of each sentence in the text (longer sentences tend to be more complex) and the length of each word in the text in syllables (longer words tend to be harder to read): the lower the metric, the more complex the language): WHY NOT POSSIBLE?

```{r Plot_AvgSentenceLength, echo=FALSE, fig.height=5, fig.width=13, message=FALSE, warning=FALSE}
# average sentence length
df$Text_punct = str_replace_all(df$Text,"!",".")

get_avgsentlen <- function(Text,bound){
  lapply(Text, function(t){
    splitted = unlist(strsplit(t, "(?<=\\.)", perl = T))
    vals_per_sentence = str_count(splitted,bound)
    avg_sentence_length_chars = sum(vals_per_sentence)/length(vals_per_sentence)
  })
}

df$AvgSentenceLength_Words = unlist(get_avgsentlen(df$Text_punct,boundary("word")))
# df$AvgSentenceLength_Chars = unlist(get_avgsentlen(df$Text_punct,"."))

toplot = df

plot_avgSentence_Words = toplot %>%
  select(PriceCategory,AvgSentenceLength_Words) %>%
  group_by(PriceCategory) %>%
  summarise(AvgAvgSentenceLength_Words = mean(AvgSentenceLength_Words), ci.low=ci.low(AvgSentenceLength_Words), ci.high=ci.high(AvgSentenceLength_Words)) %>%
  add_column(YMin=.$AvgAvgSentenceLength_Words-.$ci.low)  %>%
  add_column(YMax=.$AvgAvgSentenceLength_Words+.$ci.high)

# plot_avgSentence_Chars = toplot %>%
#   select(PriceCategory,AvgSentenceLength_Chars) %>%
#   group_by(PriceCategory) %>%
#   summarise(AvgAvgSentenceLength_Chars = mean(AvgSentenceLength_Chars), ci.low=ci.low(AvgSentenceLength_Chars), ci.high=ci.high(AvgSentenceLength_Chars)) %>%
#   add_column(YMin=.$AvgAvgSentenceLength_Chars-.$ci.low)  %>%
#   add_column(YMax=.$AvgAvgSentenceLength_Chars+.$ci.high)

binaryPlot_words = ggplot(plot_avgSentence_Words,aes(x=PriceCategory,y=AvgAvgSentenceLength_Words)) +
  geom_col(fill="#4D8069", width=.5) +
  xlab("Price") +
  ylab("Average Sentence Length") +
  geom_errorbar(aes(ymin=YMin,ymax=YMax),width=.25,color="grey")

# binaryPlot_chars = ggplot(plot_avgSentence_Chars,aes(x=PriceCategory,y=AvgAvgSentenceLength_Chars)) +
#   geom_col(fill="#4D8069", width=.5) +
#   xlab("Price") +
#   ylab("Average Sentence Length") +
#   geom_errorbar(aes(ymin=YMin,ymax=YMax),width=.25,color="grey")
...
# grid.arrange(binaryPlot_words,binaryPlot_chars,ncol=2)
```
...
```{r PhrasePlotGrid, echo=FALSE, fig.height=5, fig.width=15}
...
```{r}
# ggplot(df,aes(x=Price,y=HealthPhrases))+
#   # geom_point(aes(color=PriceCategory)) +
#   geom_point(aes(color=PriceCategory)) +
#   scale_fill_manual(values = c("yellow","green")) +
#   geom_smooth(method="lm",color="#66c2a5")
# 
# ggplot(df,aes(x=Price,y=DistinctivePhrases))+
#   geom_point(color="#fc8d62") +
#   geom_smooth(method="lm",color="#fc8d62")

ggplot(df,aes(x=Price,y=AuthenticityPhrases,color=PriceCategory))+
  geom_point() +
