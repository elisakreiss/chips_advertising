...
- types of phrases that were looked at
    ...
    - **ComparativePhrases**, e.g., more or less, suffixes like –er, or superlative words (most, least, best, finest), "unique", "less fat than other leading brands", "best in America", "in a class of their own", "a crunchy bite you won't find in any other chip"
...
- What the enriched data frame will look like (rows): 
    - Product_name, Price, PricePer100g, Weight, Text, PriceCategory, TotalWordCount, AverageWordLength, HealthyPhrases, NaturalnessPhrases, ComparativePhrases, NegativeMarkers, AuthenticityPhrases
    
## Notes about text put in data frame
- not included:
    - ingredients, contact details, instructions, Umfragen
...
- maybe also check for "du" or "Sie" (or in general -- directly address reader?) -- probably not important
...
df$HealthyPhrases = str_count(df$Text,pattern = "\\b(hohen anteil an ungesättigten fettsäuren|reich an einfach ungesättigten fettsäuren|ohne gluten|glutenfrei|gluten free|vegetariergeeignet|ohne hefeextrakt|ohne konservierungsstoffe)\\b")
df$NaturalnessPhrases = str_count(df$Text,pattern = "((\\b(ohne künstliche aromen|ohne konservierungsstoffe|ohne zusatz von geschmacksverstärkenden zusatzstoffen|100%|ohne geschmacksverstärker|von natur aus|natural)\\b)|(\\b(bio|biologisch|einfach|natürlich|organic|pur|rein|ungehärtet|unverfälscht)))")
# like this "besonderesen" is possible
df$ComparativePhrases = str_count(df$Text,pattern = "((\\b(unverwechselbar|nr 1|longer fried than regular chips|more|erstklassige Zutaten)\\b)|(außergewöhnlich|besonder|beste|extra|feinste|größte|höchste))")
df$NegativeMarkers = str_count(df$Text,pattern = "(((frei)\\b)|(\\b(ohne)\\b)|(\\b(un)))")
# recognizes "in 30 generation" as well
df$AuthenticityPhrases = str_count(df$Text,pattern = "\\b(deutschen anbauregionen|vertragsbauern|deutsche(n?) hersteller(n?)|aus deutschland|aus den niederlanden oder deutschland|dorfchips|aus eigenem anbau|aus eigener produktion|familienbetrieb|familieneigenen kartoffelhof|in 3. generation|hand cooked|heimat|johannings kartoffelhof|landkartoffelchips|ländlich-rustikal|landwirt|lüneburger heide, oberpfalz und niederbayern|regionale kartoffeln|aus den regionen|reinheitsgebot|nach alter tradition|aus unserem eigenen vertragsanbau|wochenmarkt|für die menschen von hier)\\b")
df$LicensePhrases = str_count(df$Text,pattern = "\\b(garantie|garantiert|kontrolliert|kontrolliertem|qualifikationskontrolle|qualität|qualitäts-garantie|markenqualität|reinheitsgebot)\\b")

# think about KettleChips and HandCookedChips, because the text is only English
# think about Funny Chipsfrisch and Trafo, since they were on discount price
# df = droplevels(df[!df$Product_name=="KettleChips",])
...
Plot total word count against chips prices

```{r echo=FALSE}
toplot = df

ggplot(toplot,aes(x=Price,y=TotalWordCount,label=Product_name)) +
...
```{r echo=FALSE}
plot_LicensePhrases = df %>%
                 select(PriceCategory,LicensePhrases) %>%
                 group_by(PriceCategory) %>%
                 summarise(AvgLicensePhrases = mean(LicensePhrases), ci.low=ci.low(LicensePhrases), ci.high=ci.high(LicensePhrases))
plot_LicensePhrases$YMin = plot_LicensePhrases$AvgLicensePhrases - plot_LicensePhrases$ci.low
plot_LicensePhrases$YMax = plot_LicensePhrases$AvgLicensePhrases + plot_LicensePhrases$ci.high

ggplot(plot_LicensePhrases,aes(x=PriceCategory,y=AvgLicensePhrases)) +
  geom_col(fill="#ff841f", width=.5) +
  geom_errorbar(aes(ymin=YMin,ymax=YMax),width=.25,color="grey")
```
...
```{r echo=FALSE}
plot_Phrases = df %>%
               gather(HealthyPhrases,NaturalnessPhrases,ComparativePhrases,NegativeMarkers,AuthenticityPhrases,LicensePhrases,key = "Phrases",value = "Occurrences") %>%
...
ggplot(plot_Phrases,aes(x=PriceCategory,y=AvgOccurrences,fill=Phrases)) +
  ...
  scale_fill_manual(values = c("#330808","#f1fba6","#ffd162","#ff841f","#e50000","green")) +
...
```{r echo=FALSE}
relativePhrases = df %>%
                  select(PriceCategory,HealthyPhrases,NaturalnessPhrases,ComparativePhrases,NegativeMarkers,AuthenticityPhrases,LicensePhrases) %>%
